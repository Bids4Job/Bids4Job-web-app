...
public class ContractDAO {
	...
	/**
	 * Finds all Contracts in the database from a specified Simple User.
	 * (contract_id, pro_username, amount, rating, contract_date)
	 *
	...
	public CachedRowSet findDetailsBySimpleUserID(int simpleUserID)
		...
		String sql = "SELECT a." + CONTRACT_ID + ", a." + CONTRACT_TIME + ", a." + PRO_RATING
				+ ", b.pro_user_id, b.amount, e.pro_username FROM " + CONTRACT_TABLE + " as a INNER JOIN bid as b ON a."
				+ BID_ID + " = b." + BID_ID + " INNER JOIN pro_user as e ON e.pro_user_id = b.pro_user_id"
	...
	public CachedRowSet findDetailsByProfessionalUserID(int professionalUserID)
		...
		try {
			conn = DaoUtils.getConnection();
			preStmt = conn.prepareStatement(sql);
			preStmt.setInt(1, professionalUserID);
			rs = preStmt.executeQuery();
			crs.populate(rs);
		} finally {
			DaoUtils.closeResources(rs, preStmt, conn);
	...
	public CachedRowSet findRatingAndVotesByProUserID(int proUserID)
			throws ClassNotFoundException, SQLException, InstantiationException, IllegalAccessException {
		CachedRowSet crs = new CachedRowSetImpl();
		String sql = "SELECT AVG(" + CONTRACT_TABLE + "." + PRO_RATING + ") AS " + PRO_RATING + " , COUNT( "
				+ CONTRACT_TABLE + "." + PRO_RATING + ") AS votes FROM " + CONTRACT_TABLE
				+ " INNER JOIN bid ON bid.bid_ID = " + CONTRACT_TABLE + "." + BID_ID + " WHERE bid.pro_user_ID = ?;";
		this.prepareResources();
		try {
			conn = DaoUtils.getConnection();
			preStmt = conn.prepareStatement(sql);
